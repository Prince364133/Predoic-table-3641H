// Data by https://github.com/Bowserinator/Periodic-Table-JSON/blob/master/PeriodicTableJSON.json
const elements = [
  {
    name: "Hydrogen",
...
const legend = document.querySelector('#legend-cont');
const iframeCont = document.querySelector('#el-cat-iframe-bg');
const iframe = document.querySelector('#iframe-url');
const closeX = document.querySelector('#cat-closer');
const lgElement = document.querySelector('#el-details-cont');
const summaryCont = document.querySelector('#el-summary-cont');


function makeTable() {
  for(let i = 0; i < elements.length; i++) {
    let element = `<div class="element" id="${elements[i].symbol}" name="${elements[i].category}" data-pos="${i}">
    <div class="number">${elements[i].number}</div>
    <div class="symbol">${elements[i].symbol}</div>
    <div class="name">${elements[i].name}</div>
    <div class="atomic-mass">${elements[i].atomic_mass}</div>
  </div>`;    
    table.innerHTML += element;
  }
}

function showLgElement(e) {
  if (e.target.className === "element" || 
      e.target.parentElement.className === "element") {
    let index = e.target.getAttribute('data-pos') ||
                e.target.parentElement.getAttribute('data-pos');   
    let elDetails = elements[index];
    let elHTML = `<div class="el-details" id="${elDetails.symbol}" name="${elDetails.name}" data-pos="${elDetails.number-1}">
    <div id="el-details-number">${elDetails.number}</div>
    <div id="el-details-symbol">${elDetails.symbol}</div>
    <div id="el-details-name">${elDetails.name}</div>
    <div id="el-details-atomic-mass">${elDetails.atomic_mass}</div>
  </div>`;
    lgElement.innerHTML = elHTML;
    console.log(lgElement);
  }
}
...
function showSummary(e) {
  if (e.target.className === "element" || 
    ...
    let index = e.target.getAttribute('data-pos') ||
                e.target.parentElement.getAttribute('data-pos');   
    let elDetails = elements[index];
    let elHTML = `<p class="summary">${elDetails.summary}</p>
  <ul class="summary-list">
    <li><strong>discovered by</strong>: ${elDetails.discovered_by || 'unknown'}</li>
    <li><strong>appearance</strong>: ${elDetails.appearance || 'unknown'}</li>
    <li><strong>boil</strong>: ${elDetails.boil || 'unknown'}</li>
    <li><strong>melt</strong>: ${elDetails.melt || 'unknown'}</li>   
    <li><strong>density</strong>: ${elDetails.density || 'unknown'}</li>
    <li><strong>atomic weight</strong>: ${elDetails.atomic_mass}</li> 
  </ul>
  <div class="more-info-cont"><a class="more-info-btn" href="${elDetails.source}" title="Learn more at wikipedia." accesskey="l" target="_blank">Learn More</a></div>`;
    summaryCont.innerHTML = elHTML;
    console.log(summaryCont);
  }
}
...
legend.addEventListener('click', (e) => {
  let endpoint = e.target.id || e.target.parentNode.id;
  let url = `https://en.wikipedia.org/wiki/${endpoint}`;
  iframe.setAttribute('src', url);
  iframeCont.classList.add('showIframe');
  console.log(e.target.parentNode.id);
});

closeX.addEventListener('click', (e) => {
  iframeCont.classList.remove('showIframe');
})